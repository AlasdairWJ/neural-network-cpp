...
using MyNetwork = nn::Network<NUM_DATA_POINTS,
							  nn::FullyConnectedLayer<300>::Type,
...
{
	auto params = std::make_unique<nn::Params<MyNetwork>>();
	nn::randomiseParams<MyNetwork>(*params); // must define network type
	...
	// train
	{
		...
		auto fwd = std::make_unique<nn::Forward<BATCH_SIZE, MyNetwork>>();
		auto batchExpectation = std::make_unique<la::Matrix<BATCH_SIZE, NUM_CLASSES>>();

		auto deltaFwd = std::make_unique<nn::Forward<BATCH_SIZE, MyNetwork>>();
		auto deltaParams = std::make_unique<nn::Params<MyNetwork>>();
		auto velocity = std::make_unique<nn::Params<MyNetwork>>();

		for (int epoch = 1; epoch <= 200; epoch++)
		{
			nn::util::generate_minibatch(*input, *expectation, fwd->input, *batchExpectation);

			const double cost = nn::cost<nn::CrossEntropyCostFunction>(*batchExpectation, *fwd, *params);

			printf("epoch: %3d, cost: %.5lf\n", epoch, cost);

			nn::backward<nn::CrossEntropyCostFunction>(*batchExpectation, *fwd, *params, *deltaFwd, *deltaParams);
	...
	// test
	{
		...
		auto fwd = std::make_unique<nn::Forward<NUM_TEST_SAMPLES, MyNetwork>>();
